...
// ── Currency formatter ────────────────────────────────────────────────────────
// Always show ₦ for NGN, $ for USD, fallback to currency code
export function formatCurrency(amount, currency) {
  const c = (currency || "NGN").toUpperCase();
  const num = Number(amount || 0);
  if (c === "NGN") return `₦${num.toLocaleString("en-NG", { minimumFractionDigits: 2 })}`;
...
export function SignalPill({ active }) {
  ...
  return (
    <div style={{
      ...
      fontSize: 11, color: ok ? "#15803d" : AMBER,
      background: ok ? "#dcfce7" : AMBER_BG,
